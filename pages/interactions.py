...
layout = dbc.Container([
    ...
    #
    dbc.Row([lf.make_Subtitle('Upload Metabolite Protein Data')]),
    dbc.Row([
        dbc.Col([
    ...
    dbc.Row([lf.MPI_CollapsableTable(id)]),
...
@callback(
    Output(component_id=id('metabolite-file'),component_property='children'),
...
def update_metabolite(list_of_contents, list_of_filename, value):
    ...
    if list_of_contents is not None:
        ...
        for i in value.split('\n'):
            ...
            name = 'You have manually input protein data.'
    return html.H6(name),df.to_json(orient='split')
...
def update_metabolite_data(metabolite_data, n_clicks):
    if n_clicks > 0:
        ...
        return html.Div([
            html.H6("Uploaded metabolite data", style={"color": "blue", "font-weight": "bold"}),
...
@callback(
    Output(component_id=id('protein-file'),component_property='children'),
    ...
    State(component_id=id('upload-protein-data'),component_property='filename'),
    Input(component_id=id('protein-prefill-textarea'),component_property='value')
)

def update_protein(list_of_contents, list_of_filename, value):
...
def update_protein_data(protein_data, n_clicks):
    if n_clicks > 0:
        ...
        return html.Div([
            html.H6("Uploaded protein data",style={"color": "blue", "font-weight": "bold"}),
                    dash_table.DataTable(
                        data=df.to_dict('records'),
                        columns=[{'id': c, 'name': c} for c in df.columns],
                        fixed_columns={'headers': True, 'data': 1},
...
@callback(
    ...
    Output(component_id=id('store-mpi-result-table'),component_property='data'),
...
def update_molecular_feature(n_clicks,genome,protein_data, metabolite_data):
    if n_clicks > 0:
        ...
        return umap_df.to_json(orient='split'),tt.to_json(orient='split')
...
@callback(
    Output(component_id=id('collps_MPI_Tab'), component_property='is_open'),
    Output(component_id=id('btn_openTabMPI'), component_property='children'),
    Output(component_id=id('btn_openTabMPI'), component_property='color'),
    Input(component_id=id('btn_openTabMPI'),component_property='n_clicks'),
    State(component_id=id('collps_MPI_Tab'), component_property='is_open'),
    prevent_initial_call=True
)
def invertTabVisibility( _ , previousState):
    newState = not previousState
    if newState:
        text = 'Hide MPI Prediction Result'
        color = 'info'
    else:
        text = 'Show MPI Prediction Result'
        color = 'primary'
    return newState, text, color

@callback(
    Output(component_id=id('collps_MPI_Tab'), component_property='children'),
    Input(component_id=id('store-mpi-result-table'), component_property='data'),
    Input(component_id=id('btn_openTabMPI'), component_property='n_clicks')
)

def showMPIResultTable(data, n_clicks):
    data = pd.read_json(data, orient='split')
    if n_clicks > 0:
        return html.Div([
            html.H6("Metabolite-Protein Interaction Prediction Result",style={"color": "blue", "font-weight": "bold"}),
                    dash_table.DataTable(
